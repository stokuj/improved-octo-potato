<script>
    ...
    let total = $state(0);
    let offset = $state(0);
    let limit = 100; // Increased limit for smoother virtualization
    ...
    const VISIBLE_COUNT = 20; // Strictly show 20 items
    let scrollY = $state(0);
    let containerTop = $state(0);
    let containerRef;

    // Derived virtual indices
    ...
    // Check if we need more items when scrolling
    $effect(() => {
        if (items.length > 0) {
            updateContainerPos();
        }
...
<div class="space-y-6">
    ...
    <!-- Virtualized Table (div-based for better virtualization control) -->
            ...
            {#each visibleItems as item (item.id)}
                ...
                <div class="grid grid-cols-12 gap-4 px-4 items-center border-b border-base-100 hover:bg-primary/5 transition-colors group" style="height: {ROW_HEIGHT}px;">
                    ...
                    <div class="hidden md:block col-span-1 text-center">
                        <button class="btn btn-ghost btn-circle btn-sm hover:scale-110 active:scale-95 text-base-content/20 hover:text-primary transition-all" aria-label="Save to inventory">
