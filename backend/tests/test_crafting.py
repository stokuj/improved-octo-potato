...
async def test_calculate_multiplier_too_large_returns_422(client: AsyncClient, crafting_setup):
    sword = crafting_setup["sword"]
    resp = await client.post(f"/api/crafting/{sword.id}/calculate", json={"multiplier": 10_001})
    assert resp.status_code == 422


async def test_calculate_multiplier_zero_returns_422(client: AsyncClient, crafting_setup):
    sword = crafting_setup["sword"]
    resp = await client.post(f"/api/crafting/{sword.id}/calculate", json={"multiplier": 0})
    assert resp.status_code == 422


async def test_calculate_negative_inventory_returns_422(client: AsyncClient, crafting_setup):
    sword = crafting_setup["sword"]
    ingot = crafting_setup["ingot"]
    resp = await client.post(
        f"/api/crafting/{sword.id}/calculate",
...
async def test_calculate_has_missing_prices_when_ingredient_has_no_price(
    ...
    output = Item(name="MP Output", category=ItemCategory.WEAPONS, grade=ItemGrade.RARE, current_price=5000)
    ingredient = Item(name="MP Ingredient", category=ItemCategory.CRAFTING, grade=ItemGrade.GRAND, current_price=None)
    ...
    resp = await client.post(f"/api/crafting/{output.id}/calculate", json={"multiplier": 1})
    assert resp.status_code == 200
    data = resp.json()
