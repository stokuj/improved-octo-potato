-- pricetracker_2: Approach B — Window container + CreateChildWidget button
...
local JSONL_PATH = "../Documents/Addon/pricetracker_2/prices.jsonl"
...
local function processNextSweep()
    if #sweepQueue == 0 then
        if sweepCounter > 0 then
            X2Chat:DispatchChatMessage(1, string.format("[PT2] Saved %d prices", sweepCounter))
        end
        sweepCounter = 0
        return
    end
...
UIParent:SetEventHandler(UIEVENT_TYPE.ENTERED_WORLD, function()
    X2Chat:DispatchChatMessage(1, "[PT2] loaded - approach B: window container + child button")
    -- Create a small window first, then put a button inside it (Folio105/infotracker pattern)
    local wnd = UIParent:CreateWidget("window", "PricetrackerWnd_2", "UIParent", "")
    wnd:SetExtent(130, 32)
    wnd:AddAnchor("TOPRIGHT", "UIParent", -10, 110)
    wnd:Show(true)

    local btn = wnd:CreateChildWidget("button", "PricetrackerBtn_2", 0, true)
    btn:SetText("[PT2] Save now")
    btn:SetExtent(120, 24)
    btn:AddAnchor("CENTER", wnd, 0, 0)
    btn:Show(true)
