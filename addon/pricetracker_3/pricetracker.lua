-- pricetracker_3: Approach C — chat command "!ptscan" (no UI button)
...
local JSONL_PATH = "../Documents/Addon/pricetracker_3/prices.jsonl"
...
local function processNextSweep()
    if #sweepQueue == 0 then
        if sweepCounter > 0 then
            X2Chat:DispatchChatMessage(1, string.format("[PT3] Saved %d prices", sweepCounter))
        end
        sweepCounter = 0
        return
    end
...
-- Chat command trigger — type "!ptscan" in any chat channel
UIParent:SetEventHandler(UIEVENT_TYPE.CHAT_MESSAGE, function(channel, relation, name, message, info)
    if message == "!ptscan" then
        startManualSweep()
    end
end)

UIParent:SetEventHandler(UIEVENT_TYPE.ENTERED_WORLD, function()
    X2Chat:DispatchChatMessage(1, "[PT3] loaded - type !ptscan to trigger manual sweep")
end)
